...
# query_db.sh - Query database and run write_ans.sh for all folds
# Usage: scripts/query_db.sh <design_name>
...
echo "Querying database for design: $DESIGN_NAME"
...
for fold_dir in "${DATABASE_DIR}"/*; do
    if [ -d "$fold_dir" ]; then
        ...
        # Run write_ans.sh for this fold
        ./scripts/write_ans.sh "$DESIGN_NAME" "$ORIGINAL_DEF" "$fold_dir"
