...
def parse_best_ppad(file_path):
    """Parse best_ppa.ppad file and extract WNS, TNS, Power, HPWL, Displacement."""
    data = {}
    try:
        with open(file_path, 'r') as f:
            content = f.read()
...
def parse_best_score(file_path):
    """Parse best_ppa.score file and extract the final score."""
    try:
        with open(file_path, 'r') as f:
            content = f.read()
...
def main():
    ...
    for testcase_dir in sorted(testcases):
        ...
        best_file = testcase_dir / "best_ppa.ppad"
        score_file = testcase_dir / "best_ppa.score"
